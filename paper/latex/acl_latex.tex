...
\begin{enumerate}
    ...
    \item A three-tier statistical analysis: (a) English benchmarks show no significant correlation after Bonferroni correction; (b) multilingual benchmarks (MMLU-ProX) show significant effects via linear mixed-effects models ($z$ up to 18.5); (c) a controlled experiment with the Qwen2.5 family confirms that tokenizer quality modulates scaling benefits across languages ($\rho = 0.91$).
\end{enumerate}
...
STRR emerges as the strongest predictor (LME $z = 18.5$; Qwen2.5 $\rho = 0.91$), likely because single-token words preserve semantic unity while fragmentation forces reconstruction across positions. Fertility is marginal in the LME ($p = 0.065$) because its cross-script variance is absorbed by the random intercept. Practically, our results suggest that tokenizer developers targeting multilingual performance should prioritize STRR over raw compression: a vocabulary of $\geq$100K tokens with balanced allocation across target languages consistently achieves parity ratios below 3.0 and STRR above 0.3 for most scripts. Model developers can use TokLens to audit per-language tokenizer quality before training and identify languages likely to underperform at scale.

\section{Conclusion}

We introduced TokLens, an open-source toolkit for multilingual tokenizer evaluation, and analyzed 24 tokenizers across 15 languages. No intrinsic metric predicts English benchmarks after controlling for model size. However, multilingual analysis using MMLU-ProX reveals that STRR, CPT, and parity significantly predict per-language performance via linear mixed-effects models ($p < 0.001$), and a controlled experiment with Qwen2.5 confirms that tokenizer quality modulates scaling benefits ($\rho = 0.91$).
...
\bottomrule
\end{tabular}
\caption{Open LLM Leaderboard v2 benchmark scores for all 22 models with leaderboard entries, sorted by parameter count. All scores are sourced from the official leaderboard. MATH = MATH Lvl 5. Avg = unweighted average of all six benchmarks.}
...
\bottomrule
\end{tabular}
\caption{Linear mixed-effects model results. Each model: score $\sim$ z(metric) + log(params) + (1$|$model), fitted with REML on 77 observations (7 models, 11 languages). $\beta$: fixed effect of z-scored metric. Model random intercept variance ranges from 100 to 115. All metrics except fertility reach significance ($p < 0.05$), with STRR, CPT, NSL, and parity surviving Bonferroni correction.}
\label{tab:lme}
\end{table}

\section{Fertility Heatmap}
\label{app:fertility}
\begin{figure}[H]
\centering
...
\bottomrule
\end{tabular}
\caption{Selected Spearman correlations between TokLens metrics and benchmarks ($n=22$, 77 total comparisons). ``avg'' = average across 15 languages; ``en'' = English only. Bonferroni threshold $\alpha = 0.00065$. No correlation is statistically significant.}
\label{tab:full_corr}
\end{table}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{fig11_partial_residuals.pdf}
\caption{Left: raw English fertility vs.\ average benchmark score. Right: partial residuals after regressing out log(model parameters) from both variables. The partial correlation ($r = -0.25$, $p = 0.26$) confirms that fertility has no significant predictive power for English benchmarks beyond model scale.}
...
toklens eval meta-llama/Llama-3.1-8B \
    --langs en zh ja ar
toklens compare meta-llama/Llama-2-7b-hf \
    meta-llama/Llama-3.1-8B
\end{verbatim}
